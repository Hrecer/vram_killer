...
public class TextureUploadInterceptor {
	...
	private static final int PLACEHOLDER_SIZE = 4;
	...
	public boolean tryInterceptUpload(NativeImage image, String resourcePath, int glTextureId) {
		...
		if (cached != null && cached.compressedData != null) {
			...
			VRAMKiller.LOGGER.debug("Cache hit for texture {} ({}x{})", resourcePath, width, height);
			return true;
		}
		...
		if (diskCached != null && diskCached.compressedData != null) {
			...
			VRAMKiller.LOGGER.debug("Disk cache hit for texture {} ({}x{})", resourcePath, width, height);
			return true;
		}
