...
public class TextureUploadInterceptor {
	...
	public boolean interceptUpload(NativeImage image, String resourcePath) {
		...
		if (VRAMKillerConfigBase.Compression.asyncCompression) {
			...
			try {
				...
				java.util.concurrent.Future<CompressionResult> future = compressionEngine.submitCompression(task);
				CompressionResult result = future.get(50, java.util.concurrent.TimeUnit.MILLISECONDS);
				...
				VRAMKiller.LOGGER.debug("Sync compression timed out for {}, falling back to async", resourcePath);
			} catch (Exception e) {
	...
	private CompressionResult tryLoadFromDiskCache(String resourcePath, int width, int height, String memoryCacheKey) {
		try {
			com.vram_killer.client.cache.TextureCacheManager diskCache = 
				VRAMKillerClient.getVRAMManager().getCacheManager();
			if (diskCache == null || !diskCache.isInitialized()) return null;

			com.vram_killer.client.cache.CachedTexture cached = diskCache.getCachedTexture(
				resourcePath, 0, 0);
			if (cached == null) return null;

			java.nio.ByteBuffer data = cached.getData();
