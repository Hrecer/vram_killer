...
public class TextureCacheManager {
	...
	public void cacheTexture(String texturePath, ByteBuffer compressedData, 
							VRAMKillerConfigBase.Compression.CompressionFormat format, int width, int height,
							long sourceFileSize, long sourceLastModified) {
		cacheTexture(texturePath, null, compressedData, format, width, height, sourceFileSize, sourceLastModified);
	}

	public void cacheTexture(String texturePath, String resourcePackName, ByteBuffer compressedData, 
							VRAMKillerConfigBase.Compression.CompressionFormat format, int width, int height,
							long sourceFileSize, long sourceLastModified) {
		if (!initialized || compressedData == null) return;
		
		String cacheKey = generateCacheKey(texturePath, resourcePackName, sourceFileSize, sourceLastModified);
		
		final int dataSize = compressedData.remaining();
		final byte[] dataCopy = new byte[dataSize];
		compressedData.mark();
		compressedData.get(dataCopy);
		...
		asyncWriteExecutor.submit(() -> {
			try {
				...
				writeCacheFile(cacheFile, dataBuffer, format, width, height);
	...
	private void writeCacheFile(Path file, ByteBuffer data, VRAMKillerConfigBase.Compression.CompressionFormat format, 
	...
	private CachedTexture loadCachedData(CacheEntry entry) throws IOException {
		...
		try (FileInputStream fis = new FileInputStream(entry.cacheFile.toFile());
			...
			ByteBuffer data = ByteBuffer.allocate((int) dataSize);
			channel.read(data);
			data.flip();
			
			entry.lastAccessed = System.currentTimeMillis();
			
			return new CachedTexture(data, VRAMKillerConfigBase.Compression.CompressionFormat.values()[formatOrdinal], 
									width, height);
		}
	}
	...
	private void loadCacheIndex() {
		...
		try (DataInputStream dis = new DataInputStream(new FileInputStream(indexPath.toFile()))) {
			...
			for (int i = 0; i < count; i++) {
				CacheEntry entry = CacheEntry.readFromStream(dis);
				if (entry != null && Files.exists(entry.cacheFile)) {
					cacheIndex.put(generateCacheKey(entry.originalPath), entry);
					totalCacheSize.addAndGet(entry.cachedSize);
				}
			}
		} catch (IOException e) {
	...
	private void enforceSizeLimit() {
		...
		while (totalCacheSize.get() > maxSize && !cacheIndex.isEmpty()) {
			...
			if (VRAMKillerConfigBase.Cache.enableColdZone) {
				toRemove = findOldestEntry();
			} else {
				toRemove = findNewestEntry();
			}
			
			if (toRemove != null) {
				removeEntry(generateCacheKey(toRemove.originalPath));
			} else {
				break;
			}
		}
	...
	private static class CacheEntry {
		...
		public CacheEntry(Path cacheFile, String originalPath, VRAMKillerConfigBase.Compression.CompressionFormat format,
		...
		static CacheEntry readFromStream(DataInputStream dis) throws IOException {
			String originalPath = dis.readUTF();
			String fileName = dis.readUTF();
			VRAMKillerConfigBase.Compression.CompressionFormat format = VRAMKillerConfigBase.Compression.CompressionFormat.values()[dis.readInt()];
			int width = dis.readInt();
			int height = dis.readInt();
			long cachedSize = dis.readLong();
			long sourceFileSize = dis.readLong();
			long sourceLastModified = dis.readLong();
